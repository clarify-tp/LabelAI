 ...
 * Top navigation bar with foody theme, dark mode toggle, compare badge, user menu.
 * Enhanced with animations and food-inspired design.
 */
...
import { 
  ...
  Sparkles, Heart, TrendingUp, Award 
} from 'lucide-react'
...
export default function Navbar({ onShowShortcuts }) {
  ...
  const navItems = [
    { to: '/', icon: <ScanLine size={16}/>, label: 'Scan', gradient: 'from-green-500 to-emerald-500' },
    { to: '/compare', icon: <GitCompare size={16}/>, label: 'Compare', badge: compareQ.length, gradient: 'from-blue-500 to-cyan-500' },
    { to: '/chat', icon: <MessageCircle size={16}/>, label: 'Chat', gradient: 'from-purple-500 to-pink-500' },
    { to: '/history', icon: <History size={16}/>, label: 'History', gradient: 'from-orange-500 to-amber-500' },
  ]
        ...
        <div className="flex items-center justify-between h-16">
          ...
          <div className="hidden md:flex items-center gap-1">
            {navItems.map((item) => (
              ...
              >
                ...
                <div className="relative flex items-center gap-1.5 text-sm font-medium text-gray-600 dark:text-gray-300 group-hover:text-gray-900 dark:group-hover:text-gray-100">
                  {item.icon}
                  {item.label}
                  {item.badge > 0 && (
          ...
          <div className="flex items-center gap-2">
            ...
            <button 
              ...
              className="hidden sm:flex p-2 rounded-lg text-gray-500 hover:text-orange-600 dark:hover:text-orange-400 hover:bg-orange-50 dark:hover:bg-orange-900/20 transition-all duration-200"
            >
              <Keyboard size={18}/>
            </button>

            {/* Theme Toggle with Food Theme */}
            <button 
              onClick={() => dispatch(toggleTheme())} 
              title="Toggle theme" 
              className="p-2 rounded-lg text-gray-500 hover:text-orange-600 dark:hover:text-orange-400 hover:bg-orange-50 dark:hover:bg-orange-900/20 transition-all duration-200"
            >
            ...
            {user ? (
              <div className="relative">
                ...
                {userMenu && (
                  <div className="absolute right-0 top-full mt-2 w-56 bg-white dark:bg-gray-800 rounded-xl shadow-2xl border border-gray-200 dark:border-gray-700 py-2 z-50 animate-in slide-in-from-top-2 duration-200">
                    ...
                    <Link to="/insights" onClick={() => setUserMenu(false)} className="flex items-center gap-3 px-4 py-2.5 text-sm text-gray-700 dark:text-gray-300 hover:bg-orange-50 dark:hover:bg-orange-900/20 transition-colors">
                      <TrendingUp size={16} className="text-emerald-500"/> My Insights
                    </Link>
          ...
          <div className="flex justify-around items-center py-2">
            {navItems.map((item) => (
              <Link 
                key={item.to} 
                to={item.to} 
                className="relative flex flex-col items-center gap-0.5 py-1 px-3 rounded-lg transition-all hover:scale-105"
              >
                <div className="relative">
                  {item.icon}
                  {item.badge > 0 && (
