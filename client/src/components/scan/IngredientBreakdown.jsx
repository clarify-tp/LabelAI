 ...
 * Each harmful ingredient shows its reason. Revant-flagged items get a star.
 *
 * Props:
      ...
      <div className="p-4">
        <div className="flex flex-wrap gap-2">
          {shown.map((ing, i) => {
            ...
            const isRevant = ing.revant_concern === 'YES'
            return (
              <Tooltip key={i} text={ing.harm_reason || 'Generally safe'} position="top">
                <span className={`inline-flex items-center gap-1 px-2.5 py-1 rounded-full text-xs font-medium border cursor-help ${color}`}>
                  {isRevant && <Star size={10} className="fill-current" />}
        ...
        <div className="mt-3 pt-3 border-t border-gray-100 dark:border-gray-700 flex flex-wrap gap-3 text-xs text-gray-500 dark:text-gray-400">
          ...
          <span className="flex items-center gap-1"><Star size={10} className="text-yellow-500 fill-current"/>Revant flagged</span>
        </div>
      </div>
    </div>
  )
