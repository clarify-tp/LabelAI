 ...
 * Uses the same Redux history state as the History page — no duplicate fetch.
 * If history is empty, dispatches fetchHistory to load data.
 */
...
export default function Insights() {
  ...
  useEffect(() => {
    ...
    // Always refresh on mount so insights stay current
  ...
  // ── Derived stats ──────────────────────────────────────────────────────────
  const scores   = scans.map(s => s.base_score).filter(v => v != null && v > 0)
  ...
  const red    = scans.filter(s => s.base_score != null && (s.base_score) < 45).length
  ...
  const trendScans = [...scans].reverse().slice(-10).filter(s => s.base_score != null)
    ...
    <div className="max-w-2xl mx-auto space-y-6">
      ...
      {trendScans.length >= 2 && (
        <div className="rounded-2xl border border-gray-200 dark:border-gray-700 p-4">
          <h3 className="text-sm font-semibold text-gray-700 dark:text-gray-300 mb-3">Recent Score Trend</h3>
          <div className="flex items-end gap-1.5 h-16">
            {trendScans.map((s, i) => {
              const pct = Math.max(4, Math.round((s.base_score / 100) * 100))
              const col = s.base_score >= 70 ? 'bg-green-500' : s.base_score >= 45 ? 'bg-orange-400' : 'bg-red-500'
              return (
                <div key={i} className="flex-1 flex flex-col items-center gap-0.5">
                  <div className={`w-full rounded-t ${col}`} style={{ height: `${pct}%` }} title={`${s.base_score}/100`} />
                </div>
              )
            })}
          </div>
